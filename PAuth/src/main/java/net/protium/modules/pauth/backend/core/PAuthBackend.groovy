...
package net.protium.modules.pauth.backend.core
...
import net.protium.modules.pauth.backend.auth.PASession
import net.protium.modules.pauth.backend.auth.PAUser
import net.protium.modules.pauth.backend.database.UserManager
import net.protium.modules.pauth.backend.http.PAResponse
import net.protium.modules.pauth.backend.utils.*
...
class PAuthBackend implements IModule {
	...
	PAResponse onRequest(Request request) {
		try {
			...
			def object = [
					status: "error",
					code  : 11,
					reason: "Can not find action '${request.action}'!"
			]
	...
	private static PAResponse action$changeRank(Request request) {
		try {
			...
			PARank newRank = PARank.valueOf(((request.specialData as Map).get("rank") ?: "USR") as String)
