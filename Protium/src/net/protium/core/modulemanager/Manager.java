...
public class Manager implements ModuleManager {
	...
	private void loadAll( ) {
		...
		for (String path : modulesArr) {
			try {
				...
				logger.log(Level.SEVERE, "Load Module FAILED: Can't open jarFile: " + path, e);
				continue;
			}
			URL[] urls;
			try {
			...
			try {
				...
				logger.log(Level.SEVERE, "reloadModule FAILED: Can not create an instance of " + c.getName(), e);
				return;
			}
			newModule.onEnable();
	...
	public void unloadModule(String name) {
		...
		classes.remove(name);
		logger.info("unloadModule SUCCESS: '" + name + "' unloaded");
	}

	@Override
	public void loadModule(String name) {
		String pathToJar = Functions.pathToFile(Constant.MOD_D, name, Constant.MOD_EXT);
		//System.err.println(pathToJar);
		try {
			new JarFile(pathToJar);
		} catch (IOException e) {
			logger.log(Level.SEVERE, "reloadModule FAILED: Can't open JAR '" + pathToJar + "'", e);
			return;
		}
		URL[] urls;
		try {
			...
			logger.log(Level.SEVERE, "reloadModule FAILED: Got malformed JAR URL", e);
			return;
		}
		for (URL ur : urls) {
		...
		classes.put(name, c);
		moduleStatus.put(name, true);
		logger.info("reloadModule SUCCESS: '" + name + "' reloaded");
	}
